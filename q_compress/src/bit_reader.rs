...
use crate::bits;
use crate::constants::{BITS_TO_ENCODE_N_ENTRIES, BYTES_PER_WORD, PREFIX_TABLE_SIZE_LOG, WORD_SIZE};
...
impl BitReader {
  ...
  pub(crate) fn read_prefix_table_idx(&mut self) -> QCompressResult<(usize, usize)> {
    ...
    if n_plus_j <= WORD_SIZE {
      ...
      let bits_read = min(PREFIX_TABLE_SIZE_LOG, self.total_bits - bit_idx);
      self.j += bits_read;
      Ok((bits_read, res))
    } else {
