...
use crate::{bit_writer, ChunkConfig, ChunkMetadata, wrapped};
...
impl FileCompressor {
  ...
  pub fn write_header(&self, dst: &mut [u8]) -> PcoResult<&mut [u8]> {
    ...
    self.0.write_header(writer.rest()?)
  }
  ...
  pub fn write_footer(&self, dst: &mut [u8]) -> PcoResult<&mut [u8]> {
    let mut extension = bit_writer::make_extension_for(dst, 0);
    let mut writer = BitWriter::new(dst, &mut extension);
...
impl<U: UnsignedLike> ChunkCompressor<U> {
  ...
  pub fn write_chunk(&self, dst: &mut [u8]) -> PcoResult<&mut [u8]> {
    let mut extension = bit_writer::make_extension_for(dst, 1);
    let mut writer = BitWriter::new(dst, &mut extension);
    writer.write_aligned_bytes(&[self.dtype_byte])?;
    let dst = self.inner.write_chunk_meta(writer.rest()?)?;
